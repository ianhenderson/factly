...
.config(function($stateProvider, $urlRouterProvider, $httpProvider){
    ...
    // Interceptor to tag outgoing request headers with the oAuth access_token, and handle refreshing those tokens they expire.
    $httpProvider.interceptors.push(function($q, $injector, LocalStorage){

        return {
            'request': function(config){ 

                // For calls to /api, add access token to each request if available.
                if (RegExp(LocalStorage.get('BASE_URL') + '/api').test(config.url)) {
            ...
            'response': function(response){ // Workaround since backend returns 200's even on errors
            ...
            'responseError': function(rejection){
                ...
                // If we get a 401 error, we're assuming that it is due to an expired access_token.
                if (rejection.status === 401){

                    return $q(function(resolve, reject){

                        $injector.invoke(function($http, AuthService){ // Workaround for circular dependency of $http in AuthService (http://stackoverflow.com/questions/14681654/i-need-two-instances-of-angularjs-http-service-or-what)

                            var originalRequest = rejection.config;

                            AuthService.oAuthRefreshToken()
                                .then(function(oAuth){
                                    ...
                                    $http(originalRequest)
                                        .success(function(data, status, headers, config){
                                            ...
                                            // As part of the $httpProvider.interceptors array, this module's code is run *before* the $http.success callbacks are applied. 
                                            // Thus, we need to return an object with 'data', 'status' and 'headers' properties as that's what the callbacks expect. (see Angular 1.3.9 source lines 9355-7)
                                            var retryResponse = {
                                                data: data,
                                                status: status,
                                                headers: headers,
                                ...
                                }, function(){}); // TODO: add error handling for failed refresh attempts.
                        });
                    });
                }
            }
